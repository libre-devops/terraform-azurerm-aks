...
| <a name="input_deployment_safeguard"></a> [deployment\_safeguard](#input\_deployment\_safeguard) | AKS deployment safeguards (best-practice enforcement on workloads). Set level (Warning or Enforcement) to enable. | <pre>object({<br/>    level               = string<br/>    excluded_namespaces = optional(list(string))<br/>  })</pre> | `null` | no |
